  ...
  try {
    ...
    for (const model of models) {
      ...
      for (let i = 0; i < modelItemList.length; i++) {
        ...
        if (dataPathId === dataFmId && dataPathId === dataDiskId && dataFmId === dataDiskId) {
          let delete1 = delete item.data.path
          debug(`${id} deleted frontmatter path?: ${delete1}`)
          let delete2 = delete item.data.id
          debug(`${id} deleted frontmatter id?: ${delete2}`)
        }
