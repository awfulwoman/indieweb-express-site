const debug = require('debug')('indieweb-express-site:routes:content:types:repost')
// 🏃‍♀️💨 Express
...
router.post(`/${model.modelDir}/create`, [renderNav, session, urlencodedParser, checkAuthentication, createValidators, createSanitizers], contentController.createPost(model))
router.get(`/${model.modelDir}/:id/edit`, [renderNav, session, checkAuthentication], contentController.updateGet(model))
router.post(`/${model.modelDir}/:id/edit`, [renderNav, session, urlencodedParser, checkAuthentication, createValidators, createSanitizers], contentController.updatePost(model))
