...
const dataRoot = () => process.env['APP_DATA_DIR'] || path.join(appRoot(), 'example', 'data')
const contentRoot = () => process.env['APP_DATA_DIR'] || path.join(appRoot(), 'example', 'content')
const logDir = () => process.env['APP_LOG_DIR'] || path.join(appRoot(), 'log')
...
const siteProtocol = () => `${process.env['APP_PROTOCOL'] || 'http'}://`
const siteDomain = () => `${process.env['APP_DOMAIN'] || '127.0.0.1'}`
const siteUrl = () => `${siteProtocol()}${siteDomain()}`
const siteAuthor = () => 'Charlie Owen'
const siteLoginPath = () =>  process.env['APP_LOGIN_PATH'] || '/login'
